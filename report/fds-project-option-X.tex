...
\begin{table}
    \centering
     ...
     \hline
     neighbor\_count\_<inner radius>\_<outer radius> & integer & Number of neighbouring Munros within the given radii \\
     \hline 
     ...
     \hline 
     population\_<inner radius>\_<outer radius> & integer & The population within the given radii \\
     \hline
    \end{tabular}
    \caption{Variables}
    \label{table:1}
...
Merging the WH and DoBIH datasets was challenging. First, we created a unique key for each Munro in both tables. The key of choice was a stringified tuple consisting of name and altitude, since only these two fields are available in both datasets. Sadly, they did not always match exactly - some particular cases were so bad that they needed to be handled manually (e.g. the name “Carn Dearg” appears in DoBIH three times). For the remaining data, we performed fuzzy matching on the keys i.e. we matched a key from WH to the closest key in DoBIH based on edit-distance.  We used `difflib` for this purpose. We verified the matched key pairs by flagging any pair for which the difference in altitudes was greater than 10m or if the intersection of their names did not match either name. We also ensured that all keys were unique. We then merged the data and removed unnecessary fields such as the aforementioned keys. \\ \\
After merging, we computed the number of \textquote{neighbours} for each Munro, that is, the number of Munros located within a \textquote{ring} comprising areas located between 0 and 5 km away, 5 and 20 km away. The former represents Munros that could be reached on foot as part of a single track, while the latter other Munros that are within driving distance and could be visited during a single trip. We only considered neighbouring Munros located on the same land mass, since we assume that a climber who is to climb multiple Munros in a restricted area will not want to drive to another island to do so (e.g. Skye).
...
% TODO: no need for "as seen in Figure x" afaik it's preferable to use (Figure x) if possible

% TODO: what is the first question?
% TODO: *why* do we visualise them?
% TODO: comment on Figure 1
% TODO: split Figure 1 to a and b
To address the first question, we start by visualising the distributions of Munro ascent counts and heights. Figure~\ref{fds-project-template:fig:box_dist} shows the individual distributions of Munros in relation to the two variables:
\begin{figure} [h!]
  \centering
  \includegraphics{report/box_dist.pdf}
  \caption{Basic distributions for number of ascents and altitude}
  \label{fds-project-template:fig:box_dist}
\end{figure} \\ \\
We wish to get a summary view of both variables at once. To that end, we plot the joint distribution of altitude and ascent count (Figure~\ref{fds-project-template:fig:heatmap}). It shows that most Munros have an altitude between 900 and 1000 $\meter$, with 2500 to 3000 ascents. There exist a number of apparent outliers in the data – as was also visible in the boxplots attached to Figure~\ref{fds-project-template:fig:box_dist}.
% Fix x-axis, it absolutely must include 900 and 1000!!!
% Fix y-axis: use thousands, also must be s.t. 2500-3000 can be read off!!!
\begin{figure} [h!]
  \centering
...
\end{figure} \\ \\
% Fix y-axis: use thousands
% Fix legend: all dot sizes should be the same!
\begin{figure} [h!]
  \centering
...
% TODO: change null hypothesis to: hypothesis that the coefficient of altitude in the model equals 0 at the?
...
\end{align*}\\
Before applying Ordinary Least Squares linear regression we need to make sure that our dependent variable is not highly skewed. In order to see if that is the case, we plot the distribution of Ascent Count and notice that it is right-skewed. In order to fix this skew, we log-transform Ascent Count. This yields a distribution which is less skewed and which looks approximately normal, as seen in Figure~\ref{fds-project-template:fig:ascent_count_distribution}. \\ \\
% TODO: Split into figure a and b?
% TODO: Use histogram instead of distplot (as per David's advice!)
\begin{figure} [h!]
  \centering
...
We now apply linear regression and observe the results of statsmodels, which produces the following error message: \textquote{The condition number is large, 1.25e+04. This might indicate that there are strong multicollinearity or other numerical problems.} Since we are only using a single predictor, multicollinearity is out of the question – which means we need to deal with the numerical instability. We remedy the instability by normalising the independent variable – i.e. centering it around 0. \\ \\
...
% TODO refer to right hand plot in Figure 6
...
% TODO: drop last sentence above? it is obvious
Now that we have our subset data with all relevant features, we standardise the dataset. This helps with numerical stability because some features have a vastly different range of values (e.g. population can be in the thousands, whereas hotel\_count is in the tens). To this end, we use Scikit Learn's StandardScaler class. \\ \\
% TODO: either drop standardscaler or add normalizer in above section for consistency
...
% TODO: explain F-statistic or remove it
% TODO: what is p-value "within" 0.01 of t? t does not seem to contribute to the explanation here
After reapplying OLS, we obtain a good fit with an adjusted $R^{2}$ of 0.525 and an $F$-statistic of 45.39. In addition, we notice that all the features in model have a $p$-value within 0.01 of $t$. This indicates the obtained coefficients are reliable. \\  \\
...
% TODO: add std for each variable? since used to calculate percentage response in independent variable
\begin{table} [h!]
\centering
...
Neighbour count 5–20km &        -1.31 &     -1.81 &    -0.805 \\
    Nearest City Dist. &       -0.512 &    -0.888 &    -0.135 \\
           ...
           Pop 50–75km &     0.0000327 &  0.0000242 &  0.0000411 \\
...
% TODO: distplot -> histplot (David's advice!)
\begin{figure} [h!]
  \centering
