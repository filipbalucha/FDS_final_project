...
After scraping WH data, we noticed that the rating count varied from values as low as 22 to as high as 317. Results for Munros with fewer ratings can be considered less reliable, as the sample is smaller. To remedy this, we wish to nudge such ratings closer to the mean for all ratings. To this end, we use Bayesian average. 
...
% DECIDED: change null hypothesis to: hypothesis that this matches what's used by statsmodels, so could be less confusing
\begin{align*}
...
% TODO: motivation for OLS, introduce null hypothesis after linear regression
...
To aid numerical stability, we normalise the independent variable – i.e. center it around 0. Applying linear regression on the processed data yields the prediction seen in Figure~\ref{fds-project-template:fig:q1_prediction}. There exists a visible increasing trend in our prediction; now we need to confirm it using the values outputted by statsmodels.
\begin{figure} [h!]
  \centering
...
The $p$-value of 0.034 for altitude tells us that there is a $\approx 3.4\%$ probability that the relationship between altitude and ascent count may be due to chance. Since $0.034 < 0.05$, it allows us to reject the null hypothesis that the coefficient of altitude in the model equals 0 at the $5\%$ level. Thus, there is a statistically significant relationship between altitude and ascent count. However, we observe that at $0.016$, the $R^{2}$ value is quite low. This tells us that the model does not explain the variance in the data too well. This motivates the use of further predictors to aid our analysis.

Since we normalised the independent variable, it has mean 0. Thus for the mean Munro, the expected value of $\ln$(Ascent count) is given by the intercept. Hence the expected ascent count for a Munro of mean altitude is $e^{\beta_0}=e^{8.4286}\approx 4576$. We now wish to interpret the slope $\beta_1$ of value 0.0008. Let $x$ be the independent variable, and $x^*$ its value upon normalisation, i.e. $x^*=x-\overline{x}$. The regression is of the form $\beta_0 + \beta_1x^* = \ln(y)$ so that $\beta_0 + \beta_1x_1^* = \ln(y_1)$ and $\beta_0 + \beta_1x_2^* = \ln(y_2)$ for two observations. Then
$$\beta_1(x_2 - x_1) = \beta_1\left((x_2 - \overline{x}) - (x_1 - \overline{x})\right) = \beta_1(x_2^* - x_1^*)  = \ln(y_2) - \ln(y_1) = \ln(y_2 / y_1)$$
Consider a unit increase in $x$, i.e. $x_2-x_1=1$. This gives $e^{\beta_1} = y_2 / y_1$, which can finally be rewritten as $e^{\beta_1} - 1 = (y_2 - y_1) / y_1$. Thus, a unit increase in altitude leads the ascent count to increase by $100(e^{0.0008} - 1)\% \approx 0.08\%$. The confidence interval of $[5.57 \times 10^{-5}, 1.00\times10^{-3}]$ tells us that in $95\%$ of all samples that could be drawn the increase in ascent count will be within $[5.70\times10^{-3}, 1.00\times10^{-1}]\%$.

To validate our approach, we note that an assumption of linear regression is that the residuals are normally distributed. This appears to be the case (Figure~\ref{fds-project-template:fig:uni_residuals_dist}). Furthermore, the variance of residuals does not change as a function of the predicted values of the independent variable (Figure~\ref{fds-project-template:fig:uni_residuals_yhat}). Thus, the dataset is not heteroscedastic. Therefore, our linear model was a reasonable choice for the problem at hand.
\begin{figure} [h!]
  \centering
  ...
  \begin{minipage}[t]{.5\linewidth}
        \centering
        % TODO: Centre caption
        \subcaption{A scatterplot of residuals against predictions.}
        \label{fds-project-template:fig:uni_residuals_dist}
    \end{minipage}%
    % TODO: david doesn't use fitted - should we do that too?
    \begin{minipage}[t]{.5\linewidth}
        \centering
...
% DECIDED: explain F-statistic or remove it, favour space-saving
